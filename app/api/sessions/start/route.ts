...
export async function POST(req: NextRequest) {
  ...
  const existing = await sql`
    SELECT id FROM sessions WHERE table_no = ${table_no} AND status IN ('running', 'paused')
  `;
  ...
  const [row] = await sql`
    INSERT INTO sessions (table_no, player1_name, player2_name, status, start_time)
    VALUES (${table_no}, ${player1_name.trim()}, ${player2_name.trim()}, 'running', now())
    RETURNING *
  `;
